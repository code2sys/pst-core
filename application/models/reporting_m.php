...
class Reporting_M extends Master_M {
    ...
    public function getRevenueWithinDateRange($start_date_time, $end_date_time) {
        $query = $this->db->query("Select sum(order_transaction.amount) as cnt from `order` join (select distinct order_id from order_status where status = 'Approved') order_status on `order`.id = order_status.order_id join order_transaction on `order`.id = order_transaction.order_id where order_date >= unix_timestamp(?) and order_date <= unix_timestamp(?) ", array($start_date_time, $end_date_time));
        $cnt = 0;
