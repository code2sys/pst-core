...
<?php //echo "<pre>";foreach($category as $id => $ref){ print_r ($ref['label']);}exit; ?>
	...
	<?php if(is_array($ref)): ?>
		...
		<h4 class="expand_heading">
			...
			<?php elseif($ref['link']): ?>
				<a href="<?php echo base_url('shopping/productlist'.@$ref['link']); ?>" onclick="setMainSearch(event, 'category', '<?php echo $id; ?>');"><?php echo $ref['label']; ?><?php if(@$ref['count']): ?> (<?php echo $ref['count']; ?>) <?php endif; ?></a>
			<?php endif; ?>
		</h4>
		<?php if(isset($ref['subcats'])): ?>
