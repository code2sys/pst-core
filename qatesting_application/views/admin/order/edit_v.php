	<div class="content_wrap">
		...
		<?php echo form_open('', array('class' => 'form_standard', 'id' => 'order_info')); 
		...
		?>
			<div class="tabular_data">
				<table width="100%" cellpadding="8">
					...
					<tr>
						...
						<td>
							...
							<?php echo form_input(array('name' => 'ship_tracking_code', 
												  	                              'value' => '', 
												  	                              'id' => 'ship_tracking_code',
												  	                              'class' => 'text medium',
												  	                              'placeholder' => 'Tracking Number')); ?><br />
							FedEx: <?php echo form_radio('carrier', 'FedEx'); ?>
							UPS: <?php echo form_radio('carrier', 'UPS'); ?>
							USPS: <?php echo form_radio('carrier', 'USPS'); ?><br /><br />
							<a href="javascript:void(0);" onclick="sendTrackingEmail();" id="button">Send Email</a>
							<?php if($order['ship_tracking_code']): $codes = json_decode($order['ship_tracking_code']); 
						...
						<tr>
							...
							<div class="payment_display">
								...
								<span id="exp_date_yr_display"><?php echo @$order['ccexpyr']; ?></span>
							</div>
							<br />
							<div class="clear"></div>
							<br />
							<?php if(@$order['order_date'] > 0): // Payment Verfied
				...
				<div style="float:right">
					Select Actions <?php echo form_dropdown('actions', array('--Check then Select--', 
																			 'Update Calculations',  
																			 'Back Order', 
																			 'Shipped', 
																			 'Ready For Pick Up', 
				...
				<div class="tabular_data">
					...
					<?php $subtotal = 0.00;  if(@$order['products']): ?>
						...
						<tr>
							<td><?php echo $product['qty']; echo form_hidden('qty['.$product['partnumber'].']', $product['qty']); ?>
							</td>
							<td><?php echo $partnumber; echo form_hidden('product_sku[]', $product['partnumber']); ?></td>
							<td><?php echo $product['name'] . '<br /> ' . $product['question'] .' ::  ' . $product['answer']; ?></td>
								...
								<div class="hidden_table">
									<table style="border:none;">
										...
										<?php 
											$lowestPrice = 0;
											$totalQty = 0;
											if(@$product['distributorRecs']):
											
												foreach($product['distributorRecs'] as $distRec): 
							...
							<td><?php echo form_checkbox('action', $product['partnumber']); ?></td>
							<td><?php echo (@$product['status']) ?  $product['status'] : @$product['stock_code']; ?></td>
							<td><?php echo $product['sale']; echo form_hidden('sale['.$product['partnumber'].']', $product['sale']); $subtotal +=  $product['sale']; ?></td>
							</tr>
						<?php endforeach; ?>		
				...
				<div class="tabular_data">
					...
					<tr>
						...
						<td>Subtotal:</td>
						<td>$<?php echo number_format($subtotal, 2); $grandTotal =  $subtotal;?></td>
					</tr>
					<tr>
						<td>Tax:</td>
						<td>$<?php echo @$order['tax'] ? $order['tax'] : '0.00'; $grandTotal += @$order['tax']; ?></td>
					</tr>
					<tr>
						<td rowspan="4">
						Notes:<br />
					...
					<tr>
						<td>S/H:</td>
						<td>$<?php echo @$order['shipping'] ? $order['shipping'] : '0.00'; $grandTotal += @$order['shipping'];?></td>
					</tr>
					<tr>
						<td>Grand Total:</td>
			...
			<div style="float:right;">
				...
				<a href="javascript:void(0);" onclick="processOrder();" id="button">Process Order </a>
			</div>
