...
<div class="side_nav" style="width:15%">
	...
	<?php foreach($categories as $category): 
        ...
        <li ><a id="parent_<?php echo $category['code']; ?>" href="/admin/product/<?php echo $category['code']; ?>" onclick="liClick('<?php echo $category['code']; ?>');"><?php echo $category['name']; ?></a></li>
        <?php else: ?>
        <ul class="hide <?php echo $category['parent_code']; ?>">
          <li class="subcat"><a href="/admin/product/<?php echo $category['code']; ?>"><?php echo $category['name']; ?></a></li>
        </ul>
  ...
  function liClick(catCode)
  {
    ...
    if($('.' + catCode)[0])
    {
      if($('.' + catCode).is(":visible") == false)
        ...
        $('#parent_' + catCode).attr('href', '/admin/product/' + catCode);
    }
  }
</script>
